      ...
      <main className="max-w-md mx-auto space-y-4">
        ...
        <div className="bg-amber-50 p-4 rounded-2xl shadow-sm border-2 border-amber-200">
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              className="bg-amber-600 text-white px-4 rounded-lg font-bold"
            >
              {searching ? "..." : "搜尋"}
            </button>
          </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-md space-y-4">
          <div className="flex justify-between items-center border-b pb-2">
            <h2 className="font-bold">成交回報填寫</h2>
            <span className="text-[10px] font-bold text-amber-600">{formData.reportType}</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <input type="text" placeholder="權利人姓名 *" required value={formData.buyerName} onChange={e => setFormData({...formData, buyerName: e.target.value})} className="p-2 border rounded-lg text-sm" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <input type="number" placeholder="成交總價" required value={formData.actualPrice} onChange={e => setFormData({...formData, actualPrice: e.target.value})} className="p-2 border rounded-lg bg-red-50 text-sm font-bold" />
            <input type="number" placeholder="本次實收" required value={formData.receivedAmount} onChange={e => setFormData({...formData, receivedAmount: e.target.value})} className="p-2 border rounded-lg bg-green-50 text-sm font-bold" />
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full bg-stone-900 text-white py-4 rounded-xl font-bold shadow-lg active:bg-black">
            {loading ? "正在儲存到試算表..." : "確認提交"}
          </button>
        </form>
      </main>
    </div>
