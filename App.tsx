...
function App() {
  ...
  const handleSelectRecord = (record: any) => {
    if (!record) return;
    // 計算前次已收金額顯示在備註
    const prevReceived = record.receivedAmount || 0;
    setFormData({
      ...formData,
      reportType: '補收尾款/續收',
      ...
      // 💡 強化備註：直接把訂金金額寫進去
      notes: `續收自[${record.fromSheet}]: ${record.date} (前次已收: ${prevReceived})`
    });
    ...
    <div className="min-h-screen bg-stone-100 font-sans text-stone-800 pb-10 px-4">
      <header className="max-w-md mx-auto py-4 flex justify-between items-center border-b mb-4">
        <div>
          ...
          <p className="text-[10px] text-stone-400 font-medium tracking-wider">v7.9 | 訂金餘額可見版</p>
        </div>
      ...
      <main className="max-w-md mx-auto space-y-4">
        ...
        <div className="bg-amber-50 p-4 rounded-2xl shadow-sm border-2 border-amber-200">
          ...
          <div className="flex gap-2">
            ...
            />
            <button type="button" onClick={handleSearchClick} className="bg-amber-600 text-white px-4 rounded-lg font-bold">
              {searching ? "..." : "搜尋"}
            </button>
          </div>
          ...
          {showResults && (
            <div className="mt-2 bg-white rounded-lg shadow-inner overflow-hidden border border-amber-100">
              {searching ? (
                <div className="p-3 text-center text-stone-400 text-sm italic">搜尋中...</div>
              ) : filteredData.length > 0 ? (
                filteredData.map((r, i) => (
                  <div key={i} onClick={() => handleSelectRecord(r)} className="p-3 border-b last:border-0 active:bg-amber-100 cursor-pointer text-left">
                    <p className="font-bold text-sm text-stone-700">{r.towerId} - {r.buyerName}</p>
                    {/* 💡 這裡增加顯示「已收金額」，方便快速預覽 */}
                    <p className="text-[10px] text-stone-500">
                      來源: [{r.fromSheet}] | 成交價: {r.actualPrice} | <span className="text-amber-600 font-bold">已收: {r.receivedAmount}</span>
                    </p>
                  </div>
                ))
              ) : (
                searchQuery && !searching && <div className="p-3 text-center text-xs text-stone-400">查無資料</div>
              )}
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-md space-y-4">
          <div className="flex justify-between items-center border-b pb-2">
            <h2 className="font-bold text-sm">📋 報單詳細資訊</h2>
          ...
          <div className="grid grid-cols-2 gap-3 text-left">
            <div className="flex flex-col">
              <label className="text-[10px] text-red-400 ml-1">總成交價 (帶入)</label>
              <input type="number" required value={formData.actualPrice} onChange={e => setFormData({...formData, actualPrice: e.target.value})} className="p-2 border rounded-lg bg-red-50 text-sm font-bold text-red-700" />
            </div>
            <div className="flex flex-col">
              <label className="text-[10px] text-green-600 ml-1">本次實收 (填入尾款)</label>
